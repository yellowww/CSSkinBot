...
function getAllForSkin(name,cb) {
    ...
    for(let i=0;i<3;i++) {
        getListings([0,1], name, i*500, listings => {
            ...
            completed++;
            if(completed === 3) return cb(allListings);
        });
    }
}
...
function analyzeItem(item) {
    ...
    if(week == undefined || month == undefined || at == undefined) return;
...
function doBatch(keys, size) {
    if(size == 0) return;
    getAllForSkin(keys[currentKey%keys.length],(items) => {
